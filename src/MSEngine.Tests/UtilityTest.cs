...
namespace MSEngine.Tests
{
    public class UtilityTest
    {
        ...
        // 6 7 8
        [Theory]
        [InlineData(0, new int[] { 1, 3, 4 })]
        [InlineData(1, new int[] { 0, 2, 3, 4, 5 })]
        [InlineData(2, new int[] { 1, 4, 5 })]
        [InlineData(3, new int[] { 0, 1, 4, 6, 7 })]
        [InlineData(4, new int[] { 0, 1, 2, 3, 5, 6, 7, 8 })]
        [InlineData(5, new int[] { 1, 2, 4, 7, 8 })]
        [InlineData(6, new int[] { 3, 4, 7, })]
        [InlineData(7, new int[] { 3, 4, 5, 6, 8 })]
        [InlineData(8, new int[] { 4, 5, 7 })]
        public void AdjacentNodeIndexesFilledCorrectly(int index, int[] expectedIndexes)
        {
            var buffer = Utilities.GetAdjacentNodeIndexes(index, 9, 3);

            Assert.Equal(expectedIndexes, buffer);
        }
        ...
        public void GetAdjacentMineCounts(int nodeIndex, int expectedMineCount)
        {
            ...
            var actualMineCount = Utilities.GetAdjacentMineCount(mines, nodeIndex, 9, 3);
        ...
        {
            Engine.FillCustomBoard(stackalloc Node[9], 0, 3);
